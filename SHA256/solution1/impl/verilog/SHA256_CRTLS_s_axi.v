...
localparam
    ...
    ADDR_INPUT_R_DATA_0 = 6'h1c,
    ADDR_INPUT_R_CTRL   = 6'h20,
    ADDR_HASH_DATA_0    = 6'h24,
    ADDR_HASH_CTRL      = 6'h28,
    WRIDLE              = 2'd0,
    WRDATA              = 2'd1,
    WRRESP              = 2'd2,
    WRRESET             = 2'd3,
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_INPUT_R_DATA_0: begin
                    rdata <= int_input_r[7:0];
                end
                ADDR_HASH_DATA_0: begin
                    rdata <= int_hash[7:0];
                end
                ADDR_HASH_CTRL: begin
                    rdata[0] <= int_hash_ap_vld;
                end
            endcase
        end
    end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_INPUT_R_DATA_0)
            int_input_r[7:0] <= (WDATA[31:0] & wmask) | (int_input_r[7:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_hash_ap_vld <= 1'b0;
    else if (ACLK_EN) begin
        if (hash_ap_vld)
            int_hash_ap_vld <= 1'b1;
        else if (ar_hs && raddr == ADDR_HASH_CTRL)
            int_hash_ap_vld <= 1'b0; // clear on read
    end
end
