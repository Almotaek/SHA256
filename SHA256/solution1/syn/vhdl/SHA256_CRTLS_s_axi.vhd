...
architecture behave of SHA256_CRTLS_s_axi is
    ...
    constant ADDR_INPUT_R_DATA_0 : INTEGER := 16#1c#;
    constant ADDR_INPUT_R_CTRL   : INTEGER := 16#20#;
    constant ADDR_HASH_DATA_0    : INTEGER := 16#24#;
    constant ADDR_HASH_CTRL      : INTEGER := 16#28#;
    ...
    signal int_hash            : UNSIGNED(7 downto 0) := (others => '0');
    signal int_hash_ap_vld     : STD_LOGIC;
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_HASH_DATA_0 =>
                        rdata_data <= RESIZE(int_hash(7 downto 0), 32);
                    when ADDR_HASH_CTRL =>
                        rdata_data(0) <= int_hash_ap_vld;
                    when others =>
                        NULL;
                    end case;
                end if;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INPUT_R_DATA_0) then
                    int_input_r(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_input_r(7 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (hash_ap_vld = '1') then
                    int_hash <= UNSIGNED(hash); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (hash_ap_vld = '1') then
                    int_hash_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_HASH_CTRL) then
                    int_hash_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
